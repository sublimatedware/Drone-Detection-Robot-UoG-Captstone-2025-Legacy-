...
# Movement parameters
MAX_Z_STEPS = 60      # Maximum steps for Z axis per movement
MAX_Y_STEPS = 30      # Maximum steps for Y axis per movement
...
def calculate_movement(delta, screen_dimension, max_movement, min_movement):
    """
    Calculate movement amount based on distance from center
    delta: distance from center (pixels)
    screen_dimension: width or height of screen (pixels)
    max_movement: maximum steps to move for this axis
    min_movement: minimum steps to move for this axis
    returns: steps to move (int)
    """
    # Normalize distance (0-1) where 1 is at screen edge
    normalized_dist = abs(delta) / (screen_dimension / 2)
    
    # Apply non-linear scaling (quadratic for more precision near center)
    scaled_dist = normalized_dist ** 3
    
    # Calculate steps (minimum min_movement, maximum max_movement)
    steps = max(min_movement, int(scaled_dist * max_movement))
        ...
        for result in results[0].boxes.data:  # Access detected objects
            ...
            if conf > 0.5 and classes[int(cls)] in classes:
                ...
                if abs(delta_x) > dead_zone_x or abs(delta_y) > dead_zone_y:
                    # Calculate dynamic movement amounts
                    ...
                    # Determine direction
                    z_dir = -1 if delta_x < 0 else 1
                    y_dir = 1 if delta_y < 0 else -1
                    
                    # Calculate proposed new positions
                    proposed_z = current_z_pos + (z_dir * z_steps)
                    proposed_y = current_y_pos + (y_dir * y_steps)
                    ...
                    if within_limits:
                        ...
                        speed_Z = int(np.interp(abs(delta_x), [0, frame.shape[1]], [500, 4000]))
                        speed_Y = int(np.interp(abs(delta_y), [0, frame.shape[0]], [500, 4000]))
                        
                        # Move motors
                        board_Z.stepper_set_speed(motor_Z, speed_Z)
                        board_Y.stepper_set_speed(motor_Y, speed_Y)
                        
                        board_Z.stepper_move(motor_Z, z_dir * z_steps)
                        board_Y.stepper_move(motor_Y, y_dir * y_steps)
                        
                        current_z_pos += z_dir * z_steps
                        current_y_pos += y_dir * y_steps
