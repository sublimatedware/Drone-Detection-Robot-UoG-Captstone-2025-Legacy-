 ...
 ***Modified for higher top speed and acceleration (sick yo)
 ***Place in AppData\Local\Programs\Python\Python313\Lib\site-packages\telemetrix
...
class Telemetrix(threading.Thread):
    ...
    def stepper_set_max_speed(self, motor_id, max_speed):
        ...
        if not 1 < max_speed <= 10000:
            if self.shutdown_on_exception:
                self.shutdown()
            raise RuntimeError('stepper_set_max_speed: Speed range is 1 - 10000.')
    ...
    def stepper_set_speed(self, motor_id, speed):
        ...
        if not -10000 <= speed <= 10000:
            if self.shutdown_on_exception:
                self.shutdown()
            raise RuntimeError('stepper_set_speed: speed range is -10000 to 10000')
